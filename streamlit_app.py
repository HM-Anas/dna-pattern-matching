...
def generate_pdf_report(title, df, figures):
    ...
    story = [
        ...
        Paragraph(str(datetime.now()), styles["Normal"]),
        Spacer(1, 12)
    ]
...
if st.button("🔍 Search Pattern"):
    ...
    if not sequences:
        st.warning("⚠️ Please upload or input a DNA sequence.")
        st.stop()
    if not pattern_input:
        st.warning("⚠️ Please enter at least one pattern.")
        st.stop()
    ...
    for header, dna_sequence in sequences.items():
        ...
        fname = f"perf_{header}_{int(time.time())}.png"
        ...
        for pat in patterns_list:
            ...
            st.markdown(
                f"<div style='background-color:#112233; padding:12px; border-radius:10px; "
...
if st.session_state.results_stored is not None:
    ...
    if st.session_state.figures:
        ...
        with zipfile.ZipFile(zip_buf, "w") as z:
            for name, png_bytes in st.session_state.figures:
                z.writestr(name, png_bytes)
        zip_buf.seek(0)
        st.download_button(
            "📦 Download Charts (ZIP)",
            zip_buf.getvalue(),
